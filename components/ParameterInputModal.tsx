...
import { CanvasModule, Connection, ModuleType, DataPreview, PolicyInfoOutput, PremiumComponentOutput, AdditionalVariablesOutput, NetPremiumOutput } from '../types';
...
const ToggleSwitch: React.FC<{ checked: boolean; onChange: (checked: boolean) => void; label?: string }> = ({ checked, onChange, label }) => (
    <div className="flex items-center gap-2">
        ...
        <button
            ...
            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-900 ${checked ? 'bg-green-600' : 'bg-gray-600'}`}
        >
            <span
...
const getConnectedDataSource = (moduleId: string, portName: string, allModules: CanvasModule[], allConnections: Connection[]): DataPreview | undefined => {
    ...
    if (sourceModule?.outputData?.type === 'DataPreview') return sourceModule.outputData;
    return undefined;
};
...
const getGlobalPolicyInfoFromCanvas = (allModules: CanvasModule[]): PolicyInfoOutput | undefined => {
    ...
    const policyModuleParams = allModules.find(m => m.type === ModuleType.DefinePolicyInfo)?.parameters;
...
const getConnectedPremiumComponents = (moduleId: string, portName: string, allModules: CanvasModule[], allConnections: Connection[]): PremiumComponentOutput | undefined => {
    ...
    if (!inputConnection) return undefined;
    const sourceModule = allModules.find(m => m.id === inputConnection.from.moduleId);
    if (sourceModule?.outputData?.type === 'PremiumComponentOutput') return sourceModule.outputData;
    return undefined;
};
...
const getConnectedAdditionalVariables = (moduleId: string, portName: string, allModules: CanvasModule[], allConnections: Connection[]): AdditionalVariablesOutput | undefined => {
    ...
    if (!inputConnection) return undefined;
    const sourceModule = allModules.find(m => m.id === inputConnection.from.moduleId);
    if (sourceModule?.outputData?.type === 'AdditionalVariablesOutput') return sourceModule.outputData;
    return undefined;
};
...
const getConnectedNetPremiumOutput = (moduleId: string, portName: string, allModules: CanvasModule[], allConnections: Connection[]): NetPremiumOutput | undefined => {
    ...
    if (!inputConnection) return undefined;
    const sourceModule = allModules.find(m => m.id === inputConnection.from.moduleId);
    if (sourceModule?.outputData?.type === 'NetPremiumOutput') return sourceModule.outputData;
    return undefined;
};
...
const NetPremiumCalculatorParams: React.FC<{
    ...
    const premiumComponents = getConnectedPremiumComponents(moduleId, 'premium_components_in', allModules, allConnections);
    const additionalVars = getConnectedAdditionalVariables(moduleId, 'additional_vars_in', allModules, allConnections);
    const policyInfo = getGlobalPolicyInfoFromCanvas(allModules);
    
    const availableVars = useMemo(() => {
        const vars = [];
        if (premiumComponents) {
            vars.push(...Object.keys(premiumComponents.nnxResults).map(k => ({ key: k, type: 'NNX' })));
            vars.push({ key: 'SUMX', type: 'SUMX' });
        }
        if (additionalVars) {
            vars.push(...Object.keys(additionalVars.variables).map(k => ({ key: k, type: 'ADDITIONAL' })));
        }
        if (policyInfo) {
            ...
            if (start === end && start > 0) {
                ...
                if (text[start - 1] === ']') {
                    ...
                    if (openBracketIndex !== -1) {
                        ...
                        if (token.length > 2) {
                            e.preventDefault();
                            const newText = text.substring(0, openBracketIndex) + text.substring(start);
                            onParametersChange({ ...parameters, formula: newText });
                            
                            setTimeout(() => {
                                if(textAreaRef.current) {
                                    textAreaRef.current.focus();
                                    textAreaRef.current.setSelectionRange(openBracketIndex, openBracketIndex);
                                }
                            }, 0);
                        }
                    }
    ...
    const getVarColor = (type: string) => {
        switch (type) {
            case 'NNX': return 'bg-blue-600 hover:bg-blue-500 text-white';
            case 'SUMX': return 'bg-green-600 hover:bg-green-500 text-white';
            case 'POLICY': return 'bg-purple-600 hover:bg-purple-500 text-white';
            case 'ADDITIONAL': return 'bg-amber-600 hover:bg-amber-500 text-white';
            default: return 'bg-gray-600 hover:bg-gray-500 text-gray-200';
        }
    };
    ...
    const renderPreview = () => {
        ...
        return (
            <div className="mt-2 p-2 bg-gray-800 rounded border border-gray-700 flex flex-wrap gap-1 items-center font-mono text-sm">
                <span className="text-gray-500 text-xs w-full uppercase font-bold mb-1">Live Preview</span>
                {parts.map((part, i) => {
                    if (part.startsWith('[') && part.endsWith(']')) {
                        const key = part.slice(1, -1);
                        const variable = availableVars.find(v => v.key === key);
                        const colorClass = variable ? getVarColor(variable.type).replace('hover:', '') : 'bg-gray-600 text-gray-300'; 
                        return (
                            <span key={i} className={`${colorClass} px-1.5 py-0.5 rounded text-xs shadow-sm`}>
                                {part}
                            </span>
                        );
                    }
    ...
    return (
        <div className="space-y-4">
            <div>
                <h4 className="text-sm text-gray-400 font-bold mb-2">Available Variables (Click to Insert)</h4>
                {availableVars.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                        {availableVars.map(v => (
                            <button 
                                ...
                                className={`${getVarColor(v.type)} text-xs font-mono px-3 py-1.5 rounded-md transition-colors shadow-sm border border-white/10`}
                                title={`Insert [${v.key}]`}
                            >
                                {v.label || v.key}
                            </button>
                    ...
                    <p className="text-sm text-gray-500">Connect 'Premium Component' or 'Additional Name' to see variables.</p>
                )}
            </div>
            <div className="flex gap-4 items-start">
                <div className="flex-grow">
                    <label className="block text-sm text-gray-400 mb-1">Net Premium Formula</label>
                    <textarea 
                        ref={textAreaRef}
                        value={formula} 
            ...
            <p className="text-xs text-gray-500">
                Variables are treated as objects. Use the buttons above to insert them. Standard math operators (+, -, *, /, ()) are supported.
            </p>
        </div>
    );
};
...
const GrossPremiumCalculatorParams: React.FC<{
    ...
    const netPremiumOutput = getConnectedNetPremiumOutput(moduleId, 'net_premium_in', allModules, allConnections);
    const additionalVars = getConnectedAdditionalVariables(moduleId, 'additional_vars_in', allModules, allConnections);
    
    const availableVars = useMemo(() => {
        const vars = [];
        if (netPremiumOutput) {
            const ppName = netPremiumOutput.variables?.['PP'] !== undefined ? 'PP' : (Object.keys(netPremiumOutput.variables).pop() || 'PP');
            ...
            if (netPremiumOutput.variables) {
                vars.push(...Object.keys(netPremiumOutput.variables).filter(k => k !== 'PP' && k !== ppName).map(k => ({ key: k, type: 'INHERITED' })));
            }
        }
        if (additionalVars) {
            vars.push(...Object.keys(additionalVars.variables).map(k => ({ key: k, type: 'ADDITIONAL' })));
    ...
    const getVarColor = (type: string) => {
        switch (type) {
            case 'NET_PREMIUM': return 'bg-green-600 hover:bg-green-500 text-white';
            case 'INHERITED': return 'bg-blue-600 hover:bg-blue-500 text-white';
            case 'ADDITIONAL': return 'bg-amber-600 hover:bg-amber-500 text-white';
            default: return 'bg-gray-600 hover:bg-gray-500 text-gray-200';
        }
    };
        ...
        <div className="space-y-4">
            <div>
                ...
                {availableVars.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-500">Connect 'Net Premium Calculator' to see variables.</p>
                )}
            </div>
            <div className="flex gap-4 items-start">
                <div className="flex-grow">
                    <label className="block text-sm text-gray-400 mb-1">Gross Premium Formula</label>
                    <textarea 
                        ref={textAreaRef}
                        value={formula} 
                        onChange={(e) => handleFormulaChange(e.target.value)} 
            ...
            {renderPreview()}
            <p className="text-xs text-gray-500">
                Use [PP] for Net Premium. Inherited variables from Net Premium are also available.
            </p>
        </div>
    );
};
...
const AdditionalNameParams: React.FC<{
    ...
    const dataSource = getConnectedDataSource(moduleId, 'data_in', allModules, allConnections);
    const policyInfo = getGlobalPolicyInfoFromCanvas(allModules);
    ...
    const handleUpdateDefinition = (id: string, field: string, value: any) => {
        const newDefinitions = definitions.map((d: any) => d.id === id ? { ...d, [field]: value } : d);
    ...
    const getPreviewValue = (def: any) => {
        ...
        if (def.type === 'lookup') {
            ...
            else if (def.rowType === 'entryAgePlus') rowIndex = Number(def.customValue) || 0; // Assuming index maps to duration
            else if (def.rowType === 'custom') rowIndex = Number(def.customValue) || 0;

            if (rowIndex < 0 || rowIndex >= dataSource.rows.length) return 'Index Out of Bounds';
    ...
    if (!dataSource) return <p className="text-sm text-gray-500">Connect and run 'Nx Mx Calculator' (or any data source) to enable variable lookup.</p>;
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <div className="grid grid-cols-5 gap-2">
                    {basicValues.map((bv: any, index: number) => (
                        <div key={index} className="bg-gray-900/50 p-2 rounded-md border border-gray-600 flex flex-col gap-2">
                            <input 
                                type="text" 
                                value={bv.name} 
                                onChange={(e) => handleUpdateBasicValue(index, 'name', e.target.value)}
                            ...
                            <input 
                                type="number" 
                                value={bv.value} 
                                onChange={(e) => handleUpdateBasicValue(index, 'value', parseFloat(e.target.value) || 0)}
                                step="0.001"
            ...
            <div className="space-y-4">
                <h4 className="text-sm text-gray-400 font-bold mb-2">Additional Variable Definitions</h4>
                <div className="space-y-3">
                    {definitions.map((def: any) => (
                        <div key={def.id} className="bg-gray-900/50 p-3 rounded-md border border-gray-600 relative flex flex-col gap-2">
                            <button onClick={() => handleRemoveDefinition(def.id)} className="absolute top-1.5 right-1.5 text-gray-500 hover:text-white"><XMarkIcon className="w-4 h-4" /></button>
                            ...
                            <div className="flex gap-4 text-sm">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input 
                                        ...
                                        onChange={() => handleUpdateDefinition(def.id, 'type', 'static')}
                                ...
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input 
                                        ...
                                        onChange={() => handleUpdateDefinition(def.id, 'type', 'lookup')}
                                        className="text-blue-600 focus:ring-blue-500"
                                    />
                            ...
                            {def.type === 'static' ? (
                                <PropertyInput 
                                    ...
                                    onChange={v => handleUpdateDefinition(def.id, 'staticValue', v)} 
                                />
                            ) : (
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    />
                                    {(def.rowType === 'entryAgePlus' || def.rowType === 'custom') && (
                                        <div className="col-span-2">
                                            <PropertyInput 
                                                label={def.rowType === 'entryAgePlus' ? 'Years after Entry Age (Duration)' : 'Row Index (0-based)'} 
                                                type="number" 
                            ...
                            <div className="text-xs text-gray-400 mt-1 flex justify-between items-center bg-gray-800 px-2 py-1 rounded">
                                ...
                                <span className="font-mono font-bold text-green-400">{getPreviewValue(def)}</span>
                            </div>
                        </div>
                    ))}
                </div>
                <button onClick={handleAddDefinition} className="w-full mt-2 px-3 py-2 text-sm bg-blue-600 hover:bg-blue-500 rounded-md font-semibold">Add Variable</button>
            </div>
        </div>
    );
};
...
const LoadDataParams: React.FC<{
    ...
    const handleBrowseClick = async () => {
        if (folderHandle && (window as any).showOpenFilePicker) {
            try {
                const [fileHandle] = await (window as any).showOpenFilePicker({ startIn: folderHandle, types: [{ description: 'CSV Files', accept: { 'text/csv': ['.csv'] } }] });
                const file = await fileHandle.getFile();
                const reader = new FileReader();
                reader.onload = (e) => { const content = e.target?.result as string; onParametersChange({ source: file.name, fileContent: content }); };
                reader.readAsText(file);
            } catch (error: any) { if (error.name !== 'AbortError') fileInputRef.current?.click(); }
        } else {
    ...
    return (
        <div>
            ...
            <div className="flex gap-2 mb-4">
                <input type="text" value={parameters.source} readOnly className="flex-grow bg-gray-900 border border-gray-600 rounded px-2 py-1.5 text-sm" placeholder="No file selected" />
                <button onClick={handleBrowseClick} className="px-3 py-1.5 text-sm bg-gray-600 hover:bg-gray-500 rounded-md font-semibold">Browse...</button>
            </div>
            <div className="mt-4">
                <h4 className="text-xs text-gray-500 uppercase font-bold mb-2">Examples</h4>
                <div className="bg-gray-900 p-2 rounded-md space-y-1">
                    {SAMPLE_DATA.filter(s => s.name.includes('.csv')).map(sample => (
                        <div key={sample.name} onDoubleClick={() => handleLoadSample(sample)} className="px-2 py-1.5 text-sm rounded-md hover:bg-gray-700 cursor-pointer" title="Double-click to load">{sample.name}</div>
                    ))}
                </div>
            </div>
        </div>
...
const SelectDataParams: React.FC<{
    ...
    const [selections, setSelections] = useState<SelectionItem[]>(() => {
        ...
        const existingSelectionMap = new Map(existingSelections.map((s: any) => [s.originalName, s]));
    ...
    return (
        <div>
            <h4 className="text-sm text-gray-400 font-bold mb-2">Column Selections</h4>
            <div className="flex gap-2 mb-3">
                <button onClick={handleSelectAll} className="px-3 py-1.5 text-xs bg-gray-600 hover:bg-gray-500 rounded-md font-semibold w-full">Select All</button>
                <button onClick={handleDeselectAll} className="px-3 py-1.5 text-xs bg-gray-600 hover:bg-gray-500 rounded-md font-semibold w-full">Deselect All</button>
            </div>
            <div className="grid grid-cols-[auto,1fr] gap-x-3 items-center mb-2 px-2">
                <div/>
                <div className="grid grid-cols-2 gap-x-3">
                    <label className="text-sm text-gray-400 font-bold">Original Name</label>
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-2 panel-scrollbar">
                {selections.map((selection, index) => (
                    <div key={selection.originalName} className="grid grid-cols-[auto,1fr] gap-x-3 items-center bg-gray-900/50 p-2 rounded-md">
                        <input
                            type="checkbox"
                        ...
                        />
                        <div className="grid grid-cols-2 gap-x-3">
                            <span className="text-sm text-gray-300 truncate bg-gray-700 px-2 py-1.5 rounded-md border border-gray-600" title={selection.originalName}>{selection.originalName}</span>
                            <input
                                type="text"
                                ...
                                disabled={selection.originalName === 'Age' || selection.originalName === 'Gender'}
                            />
                        </div>
                    </div>
                ))}
...
const RateModifierParams: React.FC<{
    ...
    const handleAddCalculation = () => {
        const newCalc = { id: `rate-mod-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, newColumnName: '', formula: '' };
    ...
    const handleUpdateCalculation = (id: string, field: string, value: any) => {
        updateCalculations(calculations.map((c: any) => c.id === id ? { ...c, [field]: value } : c));
    };
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>, calcId: string) => {
        if (e.key === 'Backspace') {
            const textarea = e.currentTarget;
            const start = textarea.selectionStart;
            const end = textarea.selectionEnd;
                    ...
                    if (openBracketIndex !== -1) {
                        ...
                        if (token.length > 2) {
                            ...
                            setTimeout(() => {
                                if(textarea) {
                                    textarea.focus();
                                    textarea.setSelectionRange(openBracketIndex, openBracketIndex);
                                }
                            }, 0);
                        }
                    }
        ...
        return (
            <div className="mt-2 p-2 bg-gray-800 rounded border border-gray-700 flex flex-wrap gap-1 items-center font-mono text-sm">
                ...
                {parts.map((part, i) => {
                    if (part.startsWith('[') && part.endsWith(']')) {
                        return (
                            <span key={i} className="bg-blue-600 px-1.5 py-0.5 rounded text-xs shadow-sm text-white">
                                {part}
                            </span>
                        );
                    }
    ...
    if (!dataSource) return <p className="text-sm text-gray-500">Connect and run a data source.</p>;

    return (
        <div className="space-y-6">
            <div>
                <h4 className="text-sm text-gray-400 font-bold mb-2">Rate Modifications</h4>
                <div className="space-y-4">
                    {calculations.map((calc: any) => (
                        <div key={calc.id} className="bg-gray-900/50 p-3 rounded-md border border-gray-600 relative flex flex-col gap-3">
                            <button onClick={() => handleRemoveCalculation(calc.id)} className="absolute top-1.5 right-1.5 text-gray-500 hover:text-white"><XMarkIcon className="w-4 h-4" /></button>
                            
                            <PropertyInput label="New Column Name" value={calc.newColumnName} onChange={v => handleUpdateCalculation(calc.id, 'newColumnName', v)} />
                            
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">Formula</label>
                                <textarea 
                                    ref={el => { if(el) formulaRefs.current.set(calc.id, el); else formulaRefs.current.delete(calc.id); }}
                                    value={calc.formula} 
                                    onChange={e => handleUpdateCalculation(calc.id, 'formula', e.target.value)} 
                                ...
                                <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto panel-scrollbar bg-gray-800 p-1 rounded mb-2">
                                    <span className="text-xs text-gray-500 w-full mb-1">Click to add variable:</span>
                    ...
                    <button onClick={handleAddCalculation} className="w-full px-3 py-2 text-sm bg-blue-600/80 hover:bg-blue-600 rounded-md font-semibold">Add Modification Rule</button>
                </div>
            </div>
        </div>
    );
...
const CalculateSurvivorsParams: React.FC<{
    ...
    const numericColumns = dataSource?.columns
        .filter(c => c.type === 'number' && c.name !== 'i_prem' && c.name !== 'i_claim')
        .map(c => c.name) || [];
    ...
    const handleAddCalculation = () => {
        const newCalcId = `calc-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
        const newCalc = {
            id: newCalcId,
            name: '',
            decrementRates: []
    ...
    const handleRemoveRateFromCalc = (id: string, rate: string) => {
        const newCalculations = (calculations || []).map((calc: any) => {
            if (calc.id === id) {
                const newRates = (calc.decrementRates || []).filter((r: string) => r !== rate);
                return {
                    ...calc,
                    name: newRates.join('_'),
                    decrementRates: newRates,
    ...
    return <>
        <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <PropertySelect 
                ...
                onChange={v => onParametersChange({ ...parameters, mortalityColumn: v })} 
                options={columnOptions}
            />
        </div>

        <div>
            <h4 className="text-sm text-gray-400 font-bold mb-2">Survivors (lx) Calculations</h4>
            {numericColumns.length > 0 ? (
                <div className="space-y-2">
                    <div className="flex flex-col space-y-2">
                        {(calculations || []).map((calc: any) => {
                            const availableRates = numericColumns.filter(c => c !== ageColumn && !(calc.decrementRates || []).includes(c));
                            return (
                                <div key={calc.id} className="bg-gray-900/50 p-3 rounded-md border border-gray-600 relative flex flex-col gap-2">
                                    <button onClick={() => handleRemoveCalculation(calc.id)} className="absolute top-1.5 right-1.5 text-gray-500 hover:text-white z-10"><XMarkIcon className="w-4 h-4" /></button>
                                    
                                    <div className="flex items-center gap-2 w-full">
                                        <div className="flex items-center bg-gray-700 border border-gray-600 rounded px-2 py-1.5 text-sm h-[38px]" title={`Resulting column name: lx_${calc.name}`}>
                                        ...
                                        <div className="flex-grow bg-gray-700 p-1.5 rounded-md border border-gray-600 min-h-[38px] flex flex-wrap gap-1 content-start items-center">
                                            {(calc.decrementRates || []).length === 0 && <p className="text-xs text-gray-500 px-1">Add decrement rates...</p>}
                                            {(calc.decrementRates || []).map((rate: string) => (
                                                <div key={rate} className="flex items-center gap-1 bg-blue-600/50 text-blue-100 px-2 py-0.5 rounded text-xs h-fit">
                                                    <span>{rate}</span>
                                                    <button onClick={() => handleRemoveRateFromCalc(calc.id, rate)}><XMarkIcon className="w-3 h-3" /></button>
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="flex gap-2 flex-shrink-0">
                                            <select 
                                                ...
                                                onChange={e => setSelectedRates(prev => ({...prev, [calc.id]: e.target.value}))}
                                                className="w-40 bg-gray-700 border border-gray-600 rounded px-2 py-1.5 text-sm h-[38px] focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={availableRates.length === 0}>
                                                <option value="" disabled>-- Select Decrement Rate --</option>
                                                {availableRates.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                            </select>
                                            <button onClick={() => handleAddRateToCalc(calc.id)} disabled={!selectedRates[calc.id]} className="px-3 py-1.5 text-sm bg-gray-600 hover:bg-gray-500 rounded-md font-semibold whitespace-nowrap h-[38px] disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed">Add Rate</button>
                                        </div>
                                    </div>
                                    
                                    <div className="text-xs text-gray-500">
                                        Outputs: <span className="text-blue-300">lx_{calc.name || '?'}</span>, <span className="text-blue-300">Dx_{calc.name || '?'}</span>
                                    </div>
                                </div>
                            );
                        })}
                    </div>
                    <button onClick={handleAddCalculation} className="w-full mt-2 px-3 py-2 text-sm bg-blue-600 hover:bg-blue-500 rounded-md font-semibold">Add Calculation</button>
                </div>
            ) : (
                 <p className="text-sm text-gray-500">Connect and run a data source to select risk rate columns.</p>
            )}
        </div>
    </>;
}
...
const ClaimsCalculatorParams: React.FC<{
    ...
    const numericColumns = React.useMemo(() => {
        ...
        const excludedNames = ['age', 'sex', 'gender', 'entryage', 'i_prem', 'i_claim'];
        return dataSource.columns
            .filter(c => c.type === 'number' && !excludedNames.includes(c.name.toLowerCase()))
            .map(c => c.name);
    }, [dataSource]);

    const lxOptions = useMemo(() => numericColumns.filter(c => c.startsWith('lx_')), [numericColumns]);
    const riskOptions = useMemo(() => numericColumns.filter(c => !c.startsWith('lx_')), [numericColumns]);
    ...
    const handleUpdateCalculation = (id: string, field: 'lxColumn' | 'riskRateColumn' | 'name', value: string) => {
    ...
    return (
        <div>
            <h4 className="text-sm text-gray-400 font-bold mb-2">Claim (dx) and Commutation (Cx) Calculations</h4>
            <div className="space-y-2">
                {(calculations || []).map((calc: any) => (
                    <div key={calc.id} className="bg-gray-900/50 p-3 rounded-md border border-gray-600 relative flex flex-col gap-2">
                        <button onClick={() => handleRemoveCalculation(calc.id)} className="absolute top-1.5 right-1.5 text-gray-500 hover:text-white"><XMarkIcon className="w-4 h-4" /></button>
                        ...
                        <div className="flex items-center gap-2 w-full mb-2">
                            <div className="flex items-center bg-gray-700 border border-gray-600 rounded px-2 py-1.5 text-sm h-[38px]" title={`Resulting prefix name: dx_${calc.name || calc.riskRateColumn}`}>
                                <span className="text-gray-400 mr-1">Name:</span>
                                <input
                                    type="text"
                                    placeholder={calc.riskRateColumn || "Optional Name"}
                                    value={calc.name || ''}
                                    onChange={(e) => handleUpdateCalculation(calc.id, 'name', e.target.value)}
                        ...
                        <div className="text-xs text-gray-500">
                            Outputs: <span className="text-blue-300">dx_{calc.name || calc.riskRateColumn || '?'}</span>, <span className="text-blue-300">Cx_{calc.name || calc.riskRateColumn || '?'}</span>
                        </div>
                    </div>
                ))}
            </div>
...
const NxMxCalculatorParams: React.FC<{
    ...
    const dataSource = getConnectedDataSource(moduleId, 'data_in', allModules, allConnections);
    const dxColumns = useMemo(() => dataSource?.columns.filter(c => c.name.startsWith('Dx_')).map(c => c.name) || [], [dataSource]);
    const cxColumns = useMemo(() => dataSource?.columns.filter(c => c.name.startsWith('Cx_')).map(c => c.name) || [], [dataSource]);

    const { nxCalculations = [], mxCalculations = [] } = parameters;
    
    const updateCalculations = (field: 'nxCalculations' | 'mxCalculations', newCalculations: any[]) => {
    ...
    useEffect(() => {
        if (cxColumns.length > 0) {
            const existingBase = new Set(mxCalculations.map((c: any) => c.baseColumn));
    ...
    const handleAdd = (field: 'nxCalculations' | 'mxCalculations') => {
        ...
        const newCalc = {
            id: `${prefix}-calc-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
            baseColumn: '',
            name: '',
            active: true,
        ...
        updateCalculations(field, [...(field === 'nxCalculations' ? nxCalculations : mxCalculations), newCalc]);
    };

    const handleRemove = (field: string, id: string) => {
        const current = field === 'nxCalculations' ? nxCalculations : mxCalculations;
        updateCalculations(field as 'nxCalculations' | 'mxCalculations', current.filter((c: any) => c.id !== id));
    };

    const handleUpdate = (field: string, id: string, updatedValues: any) => {
        const current = field === 'nxCalculations' ? nxCalculations : mxCalculations;
        const newCalcs = current.map((c: any) => c.id === id ? { ...c, ...updatedValues } : c);
        updateCalculations(field as 'nxCalculations' | 'mxCalculations', newCalcs);
    };

    const handleUpdatePaymentRatio = (calcId: string, year: number, field: 'type' | 'customValue', value: any) => {
            ...
            <div>
                ...
                <div className="space-y-2">
                    {nxCalculations.map((calc: any) => (
                        <div key={calc.id} className="bg-gray-900/50 p-2 rounded-md border border-gray-600 flex items-center gap-2">
                            <div className="absolute top-1.5 right-1.5 flex items-center gap-2">
                                <ToggleSwitch 
                                    checked={calc.active !== false} 
                                    onChange={(val) => handleUpdate('nxCalculations', calc.id, { active: val })} 
                                />
                                <button onClick={() => handleRemove('nxCalculations', calc.id)} className="text-gray-500 hover:text-white"><XMarkIcon className="w-4 h-4" /></button>
                            </div>
                            <div className="flex gap-4 items-center w-full pr-20">
                                <PropertySelect label="Base Column (Dx)" value={calc.baseColumn} onChange={v => handleUpdate('nxCalculations', calc.id, { baseColumn: v, name: v.replace(/^Dx_/, '') })} options={dxColumns} />
                            </div>
                            <div className="text-xs text-gray-500">
                                Output: <span className={calc.active !== false ? "text-blue-300" : "text-gray-500 line-through"}>Nx_{calc.name || '?'}</span>
                            </div>
                        </div>
                    ))}
                    <button onClick={() => handleAdd('nxCalculations')} className="w-full px-3 py-2 text-sm bg-blue-600/80 hover:bg-blue-600 rounded-md font-semibold">Add Nx Calculation</button>
                </div>
            </div>
            ...
            <div>
                ...
                <div className="space-y-2">
                    {mxCalculations.map((calc: any) => (
                        <div key={calc.id} className="bg-gray-900/50 p-3 rounded-md border border-gray-600 relative space-y-3">
                             <div className="absolute top-1.5 right-1.5 flex items-center gap-2">
                                <ToggleSwitch 
                                    checked={calc.active !== false} 
                                    onChange={(val) => handleUpdate('mxCalculations', calc.id, { active: val })} 
                                />
                                <button onClick={() => handleRemove('mxCalculations', calc.id)} className="text-gray-500 hover:text-white"><XMarkIcon className="w-4 h-4" /></button>
                             </div>
                             
                             <PropertySelect label="Base Column (Cx)" value={calc.baseColumn} onChange={v => handleUpdate('mxCalculations', calc.id, { baseColumn: v, name: v.replace(/^Cx_/, '') })} options={cxColumns} />
                             
                             <div className="grid grid-cols-2 gap-4">
                                <PropertySelect 
                                    label="Deductible / Adjustment" 
                                    value={calc.deductibleType} 
                                    onChange={v => handleUpdate('mxCalculations', calc.id, { deductibleType: v })} 
                                    options={[
                                ...
                                {calc.deductibleType === 'custom' && (
                                    <PropertyInput label="Custom Deductible (0-1)" type="number" step="0.01" value={calc.customDeductible} onChange={v => handleUpdate('mxCalculations', calc.id, { customDeductible: v })} />
                                )}
                             </div>
                             
                             <div>
                                <label className="block text-xs text-gray-400 mb-1 font-bold">Payment Schedule (First 3 Years)</label>
                                <div className="grid grid-cols-3 gap-2">
                                    {(calc.paymentRatios || []).map((ratio: any) => (
                                        <div key={ratio.year} className="bg-gray-800 p-1.5 rounded text-xs">
                                            <div className="text-gray-500 mb-1 text-center">Year {ratio.year}</div>
                                            <select 
                                                value={ratio.type} 
                                                onChange={e => handleUpdatePaymentRatio(calc.id, ratio.year, 'type', e.target.value)}
                                            ...
                                            {ratio.type === 'Custom' && (
                                                <input 
                                                    ...
                                                    onChange={e => handleUpdatePaymentRatio(calc.id, ratio.year, 'customValue', parseFloat(e.target.value))}
                            ...
                            <div className="text-xs text-gray-500">
                                Output: <span className={calc.active !== false ? "text-blue-300" : "text-gray-500 line-through"}>Mx_{calc.name || '?'}</span>
                            </div>
                        </div>
                    ))}
                    <button onClick={() => handleAdd('mxCalculations')} className="w-full px-3 py-2 text-sm bg-blue-600/80 hover:bg-blue-600 rounded-md font-semibold">Add Mx Calculation</button>
                </div>
            </div>
        </div>
    );
...
const PremiumComponentParams: React.FC<{
    ...
    const dataSource = getConnectedDataSource(moduleId, 'data_in', allModules, allConnections);
    const nxColumns = useMemo(() => dataSource?.columns.filter(c => c.name.startsWith('Nx_')).map(c => c.name) || [], [dataSource]);
    const mxColumns = useMemo(() => dataSource?.columns.filter(c => c.name.startsWith('Mx_')).map(c => c.name) || [], [dataSource]);

    const { nnxCalculations = [], sumxCalculations = [] } = parameters;

    const updateCalculations = (field: 'nnxCalculations' | 'sumxCalculations', newCalculations: any[]) => {
        onParametersChange({ [field]: newCalculations });
    };
    ...
    const handleAdd = (field: 'nnxCalculations' | 'sumxCalculations') => {
         const newCalc = {
             ...
             ...(field === 'nnxCalculations' ? { nxColumn: '' } : { mxColumn: '', amount: 0 })
         };
         updateCalculations(field, [...(field === 'nnxCalculations' ? nnxCalculations : sumxCalculations), newCalc]);
    };

    const handleRemove = (field: 'nnxCalculations' | 'sumxCalculations', id: string) => {
        const current = field === 'nnxCalculations' ? nnxCalculations : sumxCalculations;
        updateCalculations(field, current.filter((c: any) => c.id !== id));
    };
    
    const handleUpdate = (field: 'nnxCalculations' | 'sumxCalculations', id: string, updatedValues: any) => {
        const current = field === 'nnxCalculations' ? nnxCalculations : sumxCalculations;
        const newCalcs = current.map((c: any) => c.id === id ? { ...c, ...updatedValues } : c);
    ...
    return (
        <div className="space-y-6">
            <div>
                <h4 className="text-sm text-gray-400 font-bold mb-2">NNX Components (Annuity Factors)</h4>
                <div className="space-y-2">
                    {nnxCalculations.map((calc: any) => (
                        <div key={calc.id} className="bg-gray-900/50 p-2 rounded-md border border-gray-600 flex items-center gap-2">
                            <PropertySelect label="" value={calc.nxColumn} onChange={v => handleUpdate('nnxCalculations', calc.id, { nxColumn: v })} options={nxColumns} placeholder="Select Nx" />
                            <button onClick={() => handleRemove('nnxCalculations', calc.id)} className="mt-5 text-gray-500 hover:text-white"><XMarkIcon className="w-4 h-4" /></button>
                        </div>
                    ))}
                    <button onClick={() => handleAdd('nnxCalculations')} className="w-full px-3 py-1.5 text-xs bg-blue-600/80 hover:bg-blue-600 rounded-md">Add NNX</button>
                </div>
            </div>

            <div>
                <h4 className="text-sm text-gray-400 font-bold mb-2">SUMX Components (Benefit Factors)</h4>
                <div className="space-y-2">
                     {sumxCalculations.map((calc: any) => (
                        <div key={calc.id} className="bg-gray-900/50 p-3 rounded-md border border-gray-600 relative space-y-2">
                             <button onClick={() => handleRemove('sumxCalculations', calc.id)} className="absolute top-1.5 right-1.5 text-gray-500 hover:text-white"><XMarkIcon className="w-4 h-4" /></button>
                             <PropertySelect label="Mx Column" value={calc.mxColumn} onChange={v => handleUpdate('sumxCalculations', calc.id, { mxColumn: v })} options={mxColumns} />
                             <PropertyInput label="Benefit Amount" type="number" value={calc.amount} onChange={v => handleUpdate('sumxCalculations', calc.id, { amount: v })} />
                        </div>
                    ))}
                    <button onClick={() => handleAdd('sumxCalculations')} className="w-full px-3 py-1.5 text-xs bg-blue-600/80 hover:bg-blue-600 rounded-md">Add SUMX</button>
                </div>
            </div>
        </div>
    );
...
const ScenarioRunnerParams: React.FC<{
    ...
    const handleUpdate = (id: string, field: string, value: any) => {
        updateScenarios(scenarios.map((s: any) => s.id === id ? { ...s, [field]: value } : s));
    };

    return (
        <div className="space-y-4">
             <div className="bg-blue-900/20 border border-blue-800 p-3 rounded text-sm text-blue-200">
                Define variables to vary. The pipeline will run for every combination (Cartesian product).
            </div>
            {scenarios.map((scen: any) => (
                <div key={scen.id} className="bg-gray-900/50 p-3 rounded-md border border-gray-600 relative space-y-3">
                    <button onClick={() => handleRemove(scen.id)} className="absolute top-1.5 right-1.5 text-gray-500 hover:text-white"><XMarkIcon className="w-4 h-4" /></button>
                    
                    <PropertyInput label="Scenario Variable Name" value={scen.variableName} onChange={v => handleUpdate(scen.id, 'variableName', v)} placeholder="e.g. Age" />
                    
                    <div className="grid grid-cols-2 gap-2">
                         <div>
                            <label className="block text-sm text-gray-400 mb-1">Target Module</label>
                            <select 
                                value={scen.targetModuleId} 
                                onChange={e => handleUpdate(scen.id, 'targetModuleId', e.target.value)}
                                className="w-full bg-gray-700 border border-gray-600 rounded px-2 py-1.5 text-sm"
                            >
                                <option value="" disabled>Select Module</option>
                                {targetableModules.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                            </select>
                         </div>
                         <PropertyInput label="Param Name" value={scen.targetParameterName} onChange={v => handleUpdate(scen.id, 'targetParameterName', v)} placeholder="e.g. entryAge" />
                    </div>
            ...
            <button onClick={handleAdd} className="w-full px-3 py-2 text-sm bg-blue-600 hover:bg-blue-500 rounded-md font-semibold">Add Scenario Variable</button>
        </div>
    );
};
...
export const ParameterInputModal: React.FC<ParameterInputModalProps> = ({ module, onClose, updateModuleParameters, modules, connections, projectName, folderHandle }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={onClose}>
            <div className="bg-gray-800 text-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
