...
const ParamStat: React.FC<{ label: string; value: string | number }> = ({ label, value }) => (
    <div className="flex items-baseline justify-between w-full gap-1">
        <span className="text-[8px] font-semibold text-gray-500 uppercase tracking-wider truncate shrink-0">{label}</span>
        <span className="font-mono text-[10px] font-bold text-gray-300 truncate leading-none">{value}</span>
    </div>
);
...
export const ModuleInputSummary: React.FC<ModuleInputSummaryProps> = ({ module }) => {
    ...
    const renderContent = () => {
        switch (type) {
            ...
            case ModuleType.NetPremiumCalculator:
                ...
                const formulaPreview = formula.length > 20 ? formula.substring(0, 20) + '...' : formula || '-';
                return (
                    <div className="flex flex-col gap-0.5 w-full">
                        <ParamStat label="Formula" value={formulaPreview} />
                        <ParamStat label="Var" value={formatParamValue(parameters.variableName)} />
                    </div>
                );
            case ModuleType.GrossPremiumCalculator:
                const grossFormula = parameters.formula || '';
                const grossFormulaPreview = grossFormula.length > 20 ? grossFormula.substring(0, 20) + '...' : grossFormula || '-';
                return (
                    <div className="flex flex-col gap-0.5 w-full">
            ...
            case ModuleType.PremiumComponent:
                return (
                    <div className="flex flex-col gap-0.5 w-full">
                        <ParamStat label="Type" value={formatParamValue(parameters.componentType)} />
                        <ParamStat label="Name" value={formatParamValue(parameters.componentName)} />
                    </div>
                );
            case ModuleType.NxMxCalculator:
                return (
                    <div className="flex flex-col gap-0.5 w-full">
                        <ParamStat label="Type" value={formatParamValue(parameters.calculationType)} />
                    </div>
                );
            case ModuleType.CalculateSurvivors:
                return (
                    <div className="flex flex-col gap-0.5 w-full">
                        <ParamStat label="Method" value={formatParamValue(parameters.method)} />
                    </div>
                );
            case ModuleType.ClaimsCalculator:
                return (
                    <div className="flex flex-col gap-0.5 w-full">
                        <ParamStat label="Type" value={formatParamValue(parameters.claimType)} />
                    </div>
                );
            case ModuleType.RateModifier:
                return (
                    <div className="flex flex-col gap-0.5 w-full">
                        <ParamStat label="Factor" value={formatParamValue(parameters.modifierFactor)} />
                    </div>
                );
            ...
            case ModuleType.LoadData:
                return (
                    <div className="flex flex-col gap-0.5 w-full">
                        <ParamStat label="File" value={formatParamValue(parameters.fileName)} />
                    </div>
                );
            case ModuleType.SelectData:
                return (
                    <div className="flex flex-col gap-0.5 w-full">
                        <ParamStat label="Cols" value={Array.isArray(parameters.selectedColumns) ? parameters.selectedColumns.length : '-'} />
                    </div>
                );
            default:
                ...
                if (paramKeys.length === 0) {
                    return <div className="text-[8px] text-gray-500 text-center italic">No Params</div>;
                }
                // Show first 2 parameters
                return (
                    <div className="flex flex-col gap-0.5 w-full">
                        {paramKeys.slice(0, 2).map(key => (
                            <ParamStat key={key} label={key.length > 8 ? key.substring(0, 8) + '...' : key} value={formatParamValue(parameters[key])} />
                        ))}
                    </div>
                );
        }
