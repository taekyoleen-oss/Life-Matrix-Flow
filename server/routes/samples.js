...
import db, { dbAvailable } from "../db/samples-db.js";
import fs from "fs";
...
router.get("/", (req, res) => {
  if (!dbAvailable) {
    return res.status(503).json({
      ...
      message: "better-sqlite3가 빌드되지 않았습니다. samples.json 파일을 사용하세요.",
      solution: "Run: pnpm approve-builds better-sqlite3"
    });
  }
...
router.get("/:id", (req, res) => {
  if (!dbAvailable) {
    return res.status(503).json({
      error: "Database not available",
      message: "better-sqlite3가 빌드되지 않았습니다.",
      solution: "Run: pnpm approve-builds better-sqlite3"
    });
  }
...
router.post("/", (req, res) => {
  ...
  try {
    const { filename, name, input_data, description, category, file_content } =
      req.body;
    ...
    if (error.message.includes("UNIQUE constraint")) {
      return res
        .status(409)
        .json({ error: "Sample with this filename already exists" });
    }
    res.status(500).json({ error: error.message });
  }
});
...
router.post("/import", upload.single("file"), (req, res) => {
  ...
  try {
    ...
    const name =
      data.name || data.projectName || filename.replace(/\.(lifx|json)$/i, "");
...
router.delete("/:id", (req, res) => {
  ...
  try {
    const result = db
      .prepare("DELETE FROM samples WHERE id = ?")
